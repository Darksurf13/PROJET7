...
exports.getBestRatingBooks = (req, res, next) => {
  Book.aggregate([
    ...
    .then((books) => {
      // Récupération des IDs des livres et leur note moyenne
      ...
      Book.find({ _id: { $in: bookIds } })
        .then((fetchedBooks) => {
          ...
          fetchedBooks.sort(
            (a, b) => ratingMap[b._id.toString()] - ratingMap[a._id.toString()]
          );
...
exports.modifyBook = (req, res, next) => {
  ...
  // Avant toute modification, vérification que le userId actuel correspond à celui du livre
  Book.findOne({ _id: req.params.id })
    .then((book) => {
      // Si l'utilisateur actuel n'est pas celui qui a posté le livre
        ...
        optimizeImage(inputPath, outputPath)
          .then(() => {
            ...
            fs.unlink(inputPath, (err) => {
              ...
              // Suppression de l'ancienne image associée au livre qui vient d'être remplacée
              const oldImage = book.imageUrl.split("/images/")[1];
              fs.unlink(`images/${oldImage}`, (err) => {
                if (err) {
                  console.error(
                    "Erreur lors de la suppression de l'ancienne image optimisée:",
                    err
                  );
                  return res.status(500).json({
                    error: "Failed to delete the old optimized image",
                  });
                }
...
exports.deleteBook = (req, res, next) => {
  Book.findOne({ _id: req.params.id })
    .then((book) => {
      ...
      // Suppression de l'image optimisée
      fs.unlink(`images/${filename}`, (err) => {
        if (err) {
          console.error(
            "Erreur lors de la suppression de l'image optimisée:",
            err
          );
          return res
            .status(500)
            .json({ error: "Failed to delete the optimized image" });
        }
